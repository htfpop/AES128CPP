...
int main(int argc, char *argv[]) {
    ...
    do {
        std::cout << "Please enter your key to encrypt:";               /* Prompt user for key */
    ...
    std::string mystring = argv[1];                                     /* grab string from argv[1] */
    int pos = mystring.find(".");                                    /* mark first occurance of "." for strip */
    mystring = mystring.substr(0, pos);                            /* Create substring of path and user */
    mystring += ".enc";                                                 /* concatenate .enc */
...
t_uint8 *genKeySchedule(t_uint8 *key_ptr)                                 /* This Function generates the AES keys*/
{
    ...
    for (int i = 1; i < 11; i++)                                          /* For loop to iterate over key generation */
    {
        ...
        gBox(&gBoxReturn[0], w4, i);                                        /* Calculate gBox using word 4 and current round */
        xorFunction(&gBoxReturn[0], w1, currentBytePtr);                  /* gbox ^ first word of current key then write to key arr */
        ...
        xorFunction(w1, w2, currentBytePtr);                              /* First of key ^ second word of key, then write to key arr */
        (currentBytePtr += 4);                                            /* Increment byte pointer by 4 to refrence next word */
        w2 += 16;                                                          /* Increment second word of key by 16 bytes */
        xorFunction(w2, w3, currentBytePtr);                                /* Second word of key ^ third word of key then write to key arr */
        (currentBytePtr += 4);                                            /* Increment byte pointer by 4 bytes */
        w3 += 16;                                                           /* Increment 3rd word pointer by 16 bytes */
        xorFunction(w3, w4, currentBytePtr);                                /* Third word of key ^ fourth word of key then write to key arr */
        (currentBytePtr += 4);                                            /* Increment byte pointer by 4 bytes */
...
void gBox(t_uint8 *gbox, t_uint8 *keyptr, unsigned int roundKey)          /* This function calculates the gbox for AES*/
{
    ...
    gbox[1] = *(keyptr + 1);                                              /* Second gBox Index = second byte of word 4 in key */
    gbox[2] = *(keyptr + 2);                                              /* Third gBox Index = Third byte of word 4 in key */
    gbox[3] = *(keyptr + 3);                                              /* Forth gbox Index = forth byte in word 4 in key */
...
void xorFunction(t_uint8 *arr1, t_uint8 *arr2, t_uint8 *currentBytePtr)   /* This function calculates XOR of 2 pointers then stores */
{
...
void keyAddition(t_uint8 *dataBlock, t_uint8 *currentKey) {               /* this function performs xor of data and key */
